...
Oppstart bygger imaget `sprakvask-site:local`. Det inneholder HTML, skrifter,
...
En proxy i en annen container trenger et delt Docker-nettverk. `localhost`
i den containeren peker ikke på VM-en eller på denne containeren.

Vil du ha direkte HTTP-tilgang via VM-ens IP-adresse, endrer du `.env`:
...
Bygg for VM-ens arkitektur hvis byggemaskinen har en annen arkitektur. For eksempel
...
Du trenger ingen registry-konto. Imaget er ikke publisert til et registry.

## Lokal forhåndsvisning og SEO

Fra roten av repoet virker også dette:
